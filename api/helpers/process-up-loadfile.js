...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    let files = inputs.uploadeds;
    let idcampaing = inputs.campaing;
    //console.log("----" + idcampaing)
    if (typeof files === 'undefined') {
      console.log('End Error ---> ');
      return exits.ErrorUndefinedFile();
    } else {
      console.log(' 1. <<===== Process file ======>');
      ...
      for (const iElement of files) {
        ...
        console.log('2. <-- Extrac file ---> ' + iElement.fd);
        var base = path.resolve('.');
        console.log('Path == ' + base);
        console.log('<== Path  == Zip == File ==> ' + fileUnZipAddress);
        var fileRandom = path.basename(iElement.fd);

        extractZip(
          iElement.fd,
          { dir: fileUnZipAddress + '/' + fileRandom },
          async (err) => {
            if (err) {
              console.log(
                'Hubo un error al descomprimir el archivo ' + err.message
              );
              return exits.UnCompressError(err.message);
            } else {
              let result = await sails.helpers.processSimpleUploadfile.with({
              ...
              // }
              // else
              if (typeof result !== 'undefined') {allResult.push(result);}

              //Remove compact file
              fs.unlinkSync(iElement.fd, (err, result) => {
              ...
              //   console.log("Delete file ---" + result);
              // })
              console.log(' ---- Opinion was Inserted -----');
