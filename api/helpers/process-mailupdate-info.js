...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    let infoWisePocket = inputs.infoObject;
    let fileContents = inputs.bodymail;
    ...
    try {
      ...
      if (typeof fileContents !== 'undefined') {
        ...
        if (infoWisePocket.type === 'geomap') {
          ...
          let pathToFile = 'attachments';
          let files = await fsp.readdir(appRoot + path.sep + pathToFile, {
            withFileTypes: true,
          });
          ...
          if (!fileContents.includes('geodata') || files.length === 0) {
            throw Error(
              'No es un email para actualizar información geográfica'
            );
          } else {
            console.log('---Creating GeoInformation---');
            let campainInfo = await Campaign.find({}).limit(1);

            var campaingdata = {
              campaingid: campainInfo[0].numcamp,
            };
            let resultGeo = await sails.helpers.readGeographicInformation
              .with(campaingdata)
              .intercept('NotValidGeoJsonFile', () => {
                throw Error(
                  'No es un email para actualizar información geográfica'
                );
              });
          ...
          let  objectInData = data;
          let dataArray = [];
          for (let prop in objectInData) {
            // console.log(  );
            let objInfo = {};
            // console.log("Key " + prop)
            let  value = objectInData[prop];
          ...
          let newtitle = utf8.encode(infoWisePocket.title);
          await Information.update({ title: infoWisePocket.title }).set({
            title: newtitle,
            data: dataArray,
          });
        }
        ...
        let arrayTimestamp = await TimeUpdate.find({}).limit(1);
        if (typeof arrayTimestamp !== "undefined") {
          let lastimestamp = arrayTimestamp[0].time;
          let timestamp = Date.now().toString();
          await TimeUpdate.update({ time: lastimestamp }).set({
            time: timestamp,
          });
        }
        console.log('--- End --- ');
      ...
      //if error send message to source because email isn't in YAML format
      console.log(e);
