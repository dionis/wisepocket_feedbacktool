  ...
  fn: async function (inputs, exits) {
    ...
    if (typeof configuration !== 'undefined') {
      ...
      if (typeof configuration.emailsecure !== "undefined") {
        console.log(
          " Information about emailsecure",
          configuration.emailsecure
        );
        emailconfig.emailsecure =
          configuration.emailsecure === 'false' ||
          configuration.emailsecure !== 'true'
            ? false
            : true;
      } else {
    ...
    ///Conect at Universidad de Oriente
    //https://nodemailer.com/extras/smtp-server/
    //smtp with ssl and user and password
    //see CORPUS wiky

    //'UNSEEN', 'FLAGGED'

    console.log('Email Configuration Info : ', emailconfig);
    ...
    //  let auth = {method:'LOGIN',username:'wisepocket',password:'Cibernetico2020'}
    ...
    // server.listen(465,"smtp.uo.edu.cu",function(parmeter){
    ...
    Configuration.find({ type: "smtp" })
      .limit(1)
      .exec(async function userConfiguration(err, configurationObject) {
