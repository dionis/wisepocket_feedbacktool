...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var pathToFiles =  fileUnZipAddress + path.sep + fileRandom
      ...
      fs.readdir(pathToFiles, async function(err, items) {
         console.log(JSON.stringify(items));
         console.log("File in directory")
          for (var i=0; i<items.length; i++) {
            console.log("Archivos dentro del compactado " + items[i]);
            ...
            if (path.extname(pathToFiles + path.sep +items[i] )  !== ".xml") {
              ...
              if (path.extname(pathToFiles + path.sep +items[i] ) || ".jpeg"  || ".jpg" || ".png") {
              ...
              data = fs.readFileSync(pathToFiles + path.sep + items[i], "utf-8")
           
              result = await sails.helpers.processInternetReview.with({
                       campaingid:campaing,
                       text:data,
